...
import com.example.pokemonencyclopedia.databinding.FragmentHisuiBinding
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class HisuiFragment : Fragment() {
    private lateinit var binding: FragmentHisuiBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?,
    ): View {
        binding = FragmentHisuiBinding.inflate(layoutInflater)

        val strArray = resources.getStringArray(R.array.hisui_name)
        val apolloClient = ApolloClient.Builder()
            .serverUrl("https://graphql-pokeapi.graphcdn.app/#")
            .build()

        lifecycleScope.launch(Dispatchers.Main) {
            val res = apolloClient.query(PokemonListQuery(Optional.present(7), Optional.present(898))).execute()
            ...
            binding.hisuiRecyclerView.adapter = adapter
            binding.hisuiRecyclerView.layoutManager = GridLayoutManager(context, 3)
            binding.hisuiRecyclerView.addItemDecoration(SpacesItemDecoration(10))
