...
import com.example.pokemonencyclopedia.databinding.FragmentGalarBinding
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class GalarFragment : Fragment() {
    private lateinit var binding: FragmentGalarBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?,
    ): View {
        binding = FragmentGalarBinding.inflate(layoutInflater)

        val strArray = resources.getStringArray(R.array.galar_name)
        val apolloClient = ApolloClient.Builder()
            .serverUrl("https://graphql-pokeapi.graphcdn.app/#")
            .build()

        lifecycleScope.launch(Dispatchers.Main) {
            val res = apolloClient.query(PokemonListQuery(Optional.present(89), Optional.present(809))).execute()
            ...
            binding.galarRecyclerView.adapter = adapter
            binding.galarRecyclerView.layoutManager = GridLayoutManager(context, 3)
            binding.galarRecyclerView.addItemDecoration(SpacesItemDecoration(10))
